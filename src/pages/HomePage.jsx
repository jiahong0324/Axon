import { AlertCircle, BookOpen, Calendar, MapPin, RefreshCw, X } from 'lucide-react'
        ...
        <div className="space-y-5">
          ...
          <div className="card">
            ...
            {exams.length === 0 ? <EmptyState emoji="🎉" message="No exams coming up!" /> : (
              <div className="space-y-3">
                {exams.slice(0, 3).map(exam => <ExamMiniCard key={exam.id} exam={exam} />)}
              </div>
            )}
          </div>
          <div className="card border-purple-500/30">
            ...
            {tipLoading ? <div className="skeleton h-20 rounded-xl" /> : <p className="text-sm leading-6" style={{ color: 'var(--text-secondary, #cbd5e1)' }}>{tip}</p>}
          </div>
        </div>
      </section>
    </main>
    ...
    <article className="rounded-xl border border-white/10 p-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="muted mt-1">📅 {dateLabel(exam.exam_date)}</p>
        </div>
        <div className="text-right">
          <p className={`text-2xl font-bold ${tone}`}>{days === 0 ? 'Today' : days}</p>
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{days === 0 ? '' : 'days left'}</p>
        </div>
      </div>
    </article>
  )
