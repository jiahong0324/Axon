      ...
      <section className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-md items-center">
        <div className="glass w-full rounded-2xl p-6 shadow-2xl">
          <div className="mb-6 text-center">
            ...
            <h1 className="font-heading text-3xl font-bold gradient-text">{isForgot ? 'Reset Password' : isGuest ? 'Continue as Guest' : 'Axon'}</h1>
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{isForgot ? "Enter your email to receive a reset link." : isGuest ? "Just type your name and dive right in." : 'Your study life, neatly gathered.'}</p>
          </div>
          {!isForgot && !isGuest && (
          ...
          {isGuest ? (
            <form onSubmit={submitGuest} className="space-y-4">
              <label className="block">
                ...
                <input className="input" required value={guestName} onChange={e => setGuestName(e.target.value)} placeholder="How should we call you?" />
              </label>
