...
const StudyPlanScene = ({ progress }) => {
  ...
  const planOp = useTransform(progress, [0.31, 0.32], [0, 1])
  const planY = useTransform(progress, [0.31, 0.33], [40, 0])

  // Individual card staggers based on scroll progress
  const card1Op = useTransform(progress, [0.315, 0.32], [0, 1])
  const card2Op = useTransform(progress, [0.32, 0.325], [0, 1])
  ...
  return (
    <motion.div className="absolute inset-0 flex flex-col items-center justify-center z-40 pointer-events-none px-4" style={{ opacity }}>
      ...
      <div className="relative w-full max-w-5xl h-[400px] md:h-[450px] flex items-center justify-center perspective-[1200px]">
        ...
        {/* The AI Scanner Beam */}
        <motion.div className="absolute left-0 right-0 h-8 md:h-12 bg-gradient-to-b from-transparent via-cyan-400/80 to-transparent blur-md z-50 rounded-[100%]" style={{ top: scannerY, opacity: scannerOpacity, boxShadow: '0 0 80px 20px rgba(34,211,238,0.5)' }} />
  ...
  return (
    <motion.div className="absolute inset-0 flex items-center justify-center z-50 pointer-events-auto" style={{ opacity }}>
      ...
      <motion.div className="text-center relative z-10 px-4" style={{ scale }}>
        ...
        <a href="https://axon-com.vercel.app/login" className="px-8 md:px-10 py-4 md:py-5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-full font-black text-lg md:text-2xl hover:scale-105 transition-transform shadow-[0_0_40px_rgba(0,0,0,0.2)] dark:shadow-[0_0_40px_rgba(255,255,255,0.4)] flex w-fit items-center gap-3 mx-auto">
