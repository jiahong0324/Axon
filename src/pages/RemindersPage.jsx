    ...
    <main className="main-content">
      ...
      <section className="card mb-6 border-l-4 border-l-purple-500">
        <div className="mb-4 flex flex-col justify-between gap-3 md:flex-row md:items-center">
          ...
          <button className="btn-primary" onClick={generatePlan}>Generate Today's Plan</button>
        </div>
      ...
      <section className="card">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <button className="btn-ghost" onClick={() => setShowForm(v => !v)}><Plus className="h-4 w-4" /> Add Reminder</button>
        </div>
        {showForm && (
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {items.map(item => (
              <article key={item.id} className="flex items-center gap-3 rounded-xl border border-white/10 p-4">
                ...
                <button onClick={() => updateItem(item.id, { is_active: !item.is_active })} className={`h-7 w-12 rounded-full p-1 transition-colors ${item.is_active ? 'bg-blue-500' : 'bg-slate-700'}`}><span className={`block h-5 w-5 rounded-full bg-white transition-transform ${item.is_active ? 'translate-x-5' : ''}`} /></button>
